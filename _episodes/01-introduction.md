...
This is an advanced course. Why is it advanced? We (hopefully) saw in the discussion that although many of your problems share similar characteristics, the key to the soulution lays in details. We all need our algorithms, models, analysis to run in a way that many hands make light work. When such a situation arises in a group of people, we start with a meeting discussing who does what, when do we meet again to sync up, etc. After a while you it might start to feel like that all you do is attend/sit in meetings. We will see that there are several abstractions that can make our life easier. In large parts this course will use Dask to illustrate these abstractions.
...
~~~python
nums_list = [1, 2, 3, 4] # Write input

nums_sum = 0 # Initialize output

for i in range(len(nums_list)):
  nums_sum += nums_list[i] # Add each element to the output variable

print(nums_sum) # Print output
~~~
{: .source}
